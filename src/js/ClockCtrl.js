  ...
  function ($scope, $timeout, $window, $config, $tracks) {
    ...
    $scope.pause = function (pauseStamp) {
      ...
      if ($scope.state === 'started') {
        const startTime = ($scope.pauseTime || $scope.armTime)
        const t = (startTime * 1000) - (pauseStamp - ($scope.startStamp || pauseStamp))
        $scope.time = t
        $scope.state = 'paused'
        $tracks.pause()
    ...
    $scope.tick = function () {
      if ($scope.state === 'started') {
        const now = +(new Date())
        const startTime = ($scope.pauseTime || $scope.armTime)
        const newTime = (startTime * 1000) - (now - $scope.startStamp)
